...
public class ViewAllAccountsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<AnimalDetails> lad = adh.getAnimals();
		request.setAttribute("allAnimals", lad);
		
		if(lad.isEmpty()) {
			request.setAttribute("allAnimals", " ");
		}
		
		getServletContext().getRequestDispatcher("/animals-by-owner.jsp").forward(request, response);		
	}
